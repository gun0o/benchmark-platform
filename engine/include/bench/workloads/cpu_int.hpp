...
public:
    ...
    // (8 lanes + multiplier + sink), so the effect on a 50 ms trial is unmeasurable by
    // construction - which is the point: the mechanism is uniform across workloads and its
    // cost here is known to be nil.
