...
template <class E, std::size_t N> static std::set<std::string> names(const std::array<E, N>& all) {
    ...
    for (E e : all) s.emplace(to_string(e));
    return s;
}

static std::set<std::string> enum_values(const json& def) {
    std::set<std::string> s;
    for (const auto& v : def.at("enum")) s.insert(v.get<std::string>());
    return s;
}
...
TEST(MetricSchema, UnitAndWorkloadConditionalsMatch) {
    ...
    for (const auto& clause : schema["$defs"]["result_core"]["allOf"]) {
        ...
        EXPECT_EQ(clause["then"]["properties"]["unit"]["const"], to_string(unit_of(*m))) << metric_name;
        EXPECT_EQ(clause["then"]["properties"]["workload"]["const"], to_string(workload_of(*m))) << metric_name;
